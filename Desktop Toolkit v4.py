...
def check_user_status():
    ...
    if not is_valid_username(user):
        messagebox.showwarning("Invalid Username", "Enter a valid username.")
        return
    cmd = (
...
def force_pw_change():
    ...
    if not is_valid_username(user): 
        messagebox.showwarning("Invalid Username", "Enter a valid username.")
        return
    cmd = f"Import-Module ActiveDirectory; Set-ADUser -Identity '{user}' -ChangePasswordAtLogon $true"
...
def list_user_groups():
    ...
    cmd = (
        f"Import-Module ActiveDirectory; "
        f"Get-ADUser -Identity '{user}' -Properties MemberOf | Select -ExpandProperty MemberOf"
    )
    log(f"[User] Listing groups for {user}...", "info")
    log(f"Executing: {cmd}", "info")
    output, lvl = run_powershell(cmd)
    log(output, lvl)
...
def ping_host():
    ...
    if not is_valid_hostname(hostname): 
        messagebox.showwarning("Invalid Hostname", "Enter a valid hostname.")
        return
...
def map_c_drive():
    ...
    log(f"[Host] Mapping {hostname}\\C$...", "info")
    log(f"Executing: {cmd}", "info")
    output, lvl = run_command(cmd)
    log(output, lvl)
...
def force_gpo_update():
    ...
    log(f"[Host] Forcing GPO update for {hostname}...", "info")
    log(f"Executing: {cmd}", "info")
    output, lvl = run_powershell(cmd)
    log(output, lvl)
...
def get_ip_info():
    ...
    log(f"[Host] Getting IP info for {hostname}...", "info")
    log(f"Executing: {cmd}", "info")
    output, lvl = run_powershell(cmd)
    log(output, lvl)
...
def get_system_info():
    hostname = host_entry.get().strip()
    scriptblock = (
        "Get-ComputerInfo | Select-Object OsName,OsVersion,BiosSerialNumber,CsModel,OsUptime"
    )
...
def get_storage():
    hostname = host_entry.get().strip()
    cmd = (
        f"Get-WmiObject Win32_LogicalDisk -ComputerName {hostname} -Filter 'DriveType=3' | "
        "Select-Object DeviceID, VolumeName, "
        "@{Name='FreeSpace(GB)';Expression={[math]::Round($_.FreeSpace/1GB,2)}}, "
        "@{Name='TotalSpace(GB)';Expression={[math]::Round($_.Size/1GB,2)}}"
    )
    log(f"[Host] Getting storage info for {hostname}...", "info")
    log(f"Executing: {cmd}", "info")
    output, lvl = run_powershell(cmd)
    log(output, lvl)
...
def enter_ps_session():
    ...
    log(f"[Host] Starting remote PowerShell session to {hostname}...", "info")
    log(f"Executing: {cmd}", "info")
    output, lvl = run_powershell(cmd)
    log(output, lvl)
...
debug_frame = ttk.LabelFrame(wrapper, text=" 🧩 Output Log ", padding=8)
...
# ----- User Section -----
ttk.Label(user_frame, text="Username:").grid(row=0, column=0, sticky="e")
...
for i, (label, cmd) in enumerate(user_actions):
    ...
    b.grid(row=i, column=0, padx=2, pady=3)
    user_buttons.append(b)

# ----- Host Section -----
ttk.Label(host_frame, text="Hostname:").grid(row=0, column=0, sticky="e")
...
for i, (label, cmd) in enumerate(host_actions):
    ...
    b.grid(row=r, column=c, padx=2, pady=3)
...
# Debug log box
debug_box = scrolledtext.ScrolledText(debug_frame, wrap=tk.WORD, height=18, font=("Consolas", 9))
